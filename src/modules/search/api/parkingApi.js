// parkingApi - API truy xuất thông tin bãi đỗ xe, trạng thái chỗ trống
...
const parkingApi = {
  ...
  cancelMyMonthlyTicketRequest: async (id) => (await API.put(`/api/monthly-ticket-requests/${id}/cancel`)).data,
  // Payment for monthly ticket
  createMonthlyTicketPayment: async (requestId) => (await API.post(`/api/monthly-ticket-requests/${requestId}/payment`)).data,
