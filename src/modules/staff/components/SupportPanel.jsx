...
export default function SupportPanel({ plateNumber, gateId, activeSession }) {
    ...
    if (!branchId) {
      ...
      const fetchFallbackBranch = async () => {
        try {
          ...
          console.error(
            "Failed to fetch fallback branches for support panel",
            err,
          );
        }
      };
    ...
    try {
      ...
      if (type === "LOST_CARD") {
        ...
        createdIncident = await staffApi.reportLostCard({
          ...
          cardCode:
            activeSession.cardCode || activeSession.parkingCard?.cardCode,
        });
      } else {
        createdIncident = await staffApi.reportIncident({
          type: label,
          note,
          plateNumber,
          gateId,
  ...
  const getButtonContent = () => {
    if (status === "CREATING_REPORT")
      return (
        <>
          <span className="vin-spinner" /> ĐANG TẠO BÁO CÁO
        </>
      );
    if (status === "UPLOADING_IMAGES")
      return (
        <>
          <span className="vin-spinner" /> ĐANG TẢI ẢNH LÊN...
        </>
      );
  ...
  return (
    <div
      className="vin-card bg-white shadow-sm"
      style={{ padding: "1.25rem", borderRadius: "12px", border: "none" }}
    >
      <div
        style={{
          display: "flex",
      ...
      <div className="vin-field" style={{ marginBottom: "1rem" }}>
        ...
        >
          GHI CHÚ VẬN HÀNH <span style={{ color: "#ef4444" }}>*</span>
        </label>
        <textarea
          rows={3}
        ...
        {images.length < MAX_IMAGES && (
          <div>
            <input
              ...
              disabled={
                status === "CREATING_REPORT" || status === "UPLOADING_IMAGES"
              }
              style={{ display: "none" }}
            />
            <button
              ...
              style={{
                ...
                padding: "0.5rem",
                background: "#f8fafc",
                border: "1px dashed var(--vin-border)",
                borderRadius: "8px",
      ...
      <button
        ...
        style={{
          background:
            status === "UPLOAD_FAILED" ? "#f59e0b" : "var(--vin-success)",
          color: "#ffffff",
          padding: "0.75rem",
          fontWeight: "bold",
        }}
