// PricingPage - Trang hiển thị bảng giá đỗ xe và các gói dịch vụ
import { useState, useEffect } from "react";
import { useNavigate, useLocation } from "react-router-dom";
import { toast } from "react-toastify";
import { Dropdown } from "react-bootstrap";
import { mapBranchToParkingLot } from "../../../shared/utils/mapBranch";
...
} from "../../../shared/utils/vehiclePackageValidation";

const getVietnameseVehicleTypeName = (typeName = "") => {
  ...
  return vietnameseNames[normalizedType] || typeName || "Loại xe";
};
...
export default function PricingPage() {
  ...
  const [selectedLotId, setSelectedLotId] = useState(
    () => Number(location.state?.selectedLotId) || 1,
  );
    ...
    const fetchLots = async () => {
      try {
        ...
        const branches = Array.isArray(branchesData)
          ? branchesData
          : branchesData?.content || branchesData?.data || [];
        if (branches.length > 0) {
          const mapped = branches.map(mapBranchToParkingLot);
          setLots(mapped);
          if (!mapped.some((lot) => lot.id === Number(selectedLotId)))
  ...
  const handleConfirmPayment = async () => {
    ...
    if (selectedSubVehicleId !== "new") {
      const selectedVehicle = userVehicles.find(
        (vehicle) =>
          String(vehicle.vehicleId || vehicle.id) ===
    ...
    try {
      ...
      if (paymentMethod === "vnpay" || paymentMethod === "card") {
        if (createdRequestId) {
          try {
            const payRes =
              await parkingApi.createMonthlyTicketPayment(createdRequestId);
            const pUrl =
              payRes?.paymentUrl ||
              payRes?.url ||
              payRes?.redirectUrl ||
      ...
      const msg =
        ...
        err.response?.data ||
        "Có lỗi xảy ra khi gửi yêu cầu đăng ký.";
    ...
    if (userId) {
      parkingApi
        ...
        .then((data) => {
          ...
          const uVehicles = list.filter(
            (v) => String(v.userId) === String(userId) && !v.deleted,
          );
  ...
  const currentLot =
    lots.length > 0
      ? lots.find((lot) => lot.id === Number(selectedLotId)) || lots[0]
      : null;

  const isPackage = (p) => {
    const name = p.policyName || "";
    return (
      name.startsWith("[Gói Tháng]") || name.startsWith("[Gói VIP President]")
    );
  };
  ...
  const getPackageDetails = (p) => {
    ...
    return {
      ...
      days: days,
      desc: isVip
        ? "Đặc quyền đỗ xe cao cấp và dịch vụ chăm sóc trọn gói."
        : "Tiết kiệm chi phí gửi xe định kỳ hàng tháng.",
      vehicleTypeName: p.vehicleType?.typeName || "Mọi xe",
      vehicleTypeId: p.vehicleType?.vehicleTypeId || p.vehicleTypeId,
      perks: isVip
        ? [
            "Vị trí đỗ ưu tiên gần thang máy",
            "Hỗ trợ rửa xe 2 lần/tháng",
            "Miễn phí sạc EV (áp dụng cho 50kWh đầu)",
            "Ưu tiên hỗ trợ từ Vinparking",
          ]
        : [
            "Đỗ xe không giới hạn lượt ra vào",
            "Áp dụng cho mọi vị trí đỗ phổ thông",
            "Thanh toán tự động qua App",
          ],
    };
  };

  const activeHourlyPolicies = pricePolicies.filter(
    (p) => p.active && !isPackage(p),
  );
  const activePackages = pricePolicies
    .filter((p) => p.active && isPackage(p))
    .map(getPackageDetails);

  const prices = (() => {
    const vipCarPrice = currentLot
      ? parseInt(currentLot.monthlyPrice.replace(/[^0-9]/g, ""), 10) || 2500000
      : 2500000;
    const ecoCarPrice = Math.max(1000000, vipCarPrice - 1000000);
    const ecoMotorPrice = Math.max(
      150000,
      Math.round(ecoCarPrice / 6 / 10000) * 10000,
    );
    return {
      ecoCar: ecoCarPrice,
      ecoMotor: ecoMotorPrice,
      vipCar: vipCarPrice,
      vipMotor: ecoMotorPrice * 2,
    };
  })();
  ...
  if (selectedSubVehicleId === "new") {
    currentSelectedVehicle = {
      ...
      plate: newVehicleData.licensePlate || "Xe mới",
    };
  } else {
    const v = userVehicles.find(
      (v) => String(v.vehicleId || v.id) === String(selectedSubVehicleId),
    );
    if (v) {
      const isMotor =
        v.vehicleTypeName?.toLowerCase().includes("máy") ||
        v.vehicleTypeName?.toLowerCase().includes("moto");
      currentSelectedVehicle = {
        type: isMotor ? "Motorcycle" : "Car",
        plate: v.licensePlate,
      };
    }
  }
  if (!currentSelectedVehicle)
    currentSelectedVehicle = { type: "Car", plate: "" };
  ...
  const activePlanPrice = activeSubPlan
    ? activeSubPlan.price
      ...
      : activeSubPlan.type === "Economic"
        ? currentSelectedVehicle.type === "Car"
          ? prices.ecoCar
          : prices.ecoMotor
        : currentSelectedVehicle.type === "Car"
          ? prices.vipCar
  ...
  const dropdownFilteredLots = lots.filter(
    (lot) =>
  ...
  if (!loading && lots.length === 0)
    return (
      <div className="p-5 text-center text-muted">Không có dữ liệu bãi đỗ.</div>
    );
        ...
        <div className="container d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3">
          <div>
            ...
            <p className="text-light opacity-75 small m-0">
              Đồng bộ dữ liệu thời gian thực từ hệ thống Vinparking
            </p>
          </div>
          ...
          <Dropdown onSelect={(k) => setSelectedLotId(Number(k))}>
            <Dropdown.Toggle
              as="button"
              className="btn fw-bold px-4 py-2 text-white border-0"
              style={{ backgroundColor: "#3b82f6", minWidth: "240px" }}
            >
              {currentLot.name}
            </Dropdown.Toggle>
            <Dropdown.Menu
              className="shadow-lg border-0 mt-2 p-2"
              style={{ minWidth: "280px", borderRadius: "12px" }}
            >
              <div
                className="px-2 mb-2 border-bottom pb-2"
      ...
      <div className="container mt-4">
        <nav className="mb-4 small">
          <span className="text-muted">
            Home &gt; Ho Chi Minh City &gt; {currentLot.area.split(",")[0]}{" "}
            &gt;{" "}
          </span>
              ...
              {[
                ...
                <div className="col-6 position-relative" key={i}>
                  <img
                    ...
                    alt={`img-${i}`}
                    className={`w-100 h-100 object-fit-cover rounded-3 shadow-sm ${i === 3 ? "brightness-50" : ""}`}
                    style={{ height: "166px" }}
                  />
                  {i === 3 && (
                    <div
                    ...
                    >
                      ...
                      <span className="small mt-1">Xem thêm ảnh</span>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="card border-0 shadow-sm p-4 rounded-4 mb-4 bg-white">
          <span
            className="badge bg-primary text-white fw-bold px-2 py-1 mb-2 d-table"
            style={{ fontSize: "0.75rem" }}
          >
            VINPARKING PLUS
          </span>
          <h2 className="fw-bold mb-1">
            {currentLot.name} - Bãi đỗ thông minh
          </h2>
          <p className="text-muted small">📍 {currentLot.address}</p>
          <hr className="my-3 text-muted opacity-25" />
          <p className="text-muted" style={{ lineHeight: "1.6" }}>
            Nằm tại vị trí chiến lược của khu vực {currentLot.area}, bãi đỗ xe
            thông minh {currentLot.name} được trang bị công nghệ nhận diện biển
            số tự động dựa trên AI, hệ thống camera giám sát liên tục 24/7 và hệ
            thống sạc điện EV thông minh. Đáp ứng hoàn hảo cho các dòng xe sedan
            thông thường, SUV cỡ lớn, đem đến trải nghiệm gửi xe an tâm và cao
            ...
            {[
              {
                icon: "🔋",
                label: "EV Charging",
                desc: "Có sẵn trạm sạc điện",
              },
              {
                icon: "🛡️",
                label: "24/7 Security",
                desc: "Camera & Bảo vệ tuần tra",
              },
              {
                icon: "🏠",
                label: "Indoor Parking",
                desc: "Mái che tầng hầm B2-B3",
              },
              {
                icon: "♿",
                label: "Accessible",
                desc: "Lối đi cho người khuyết tật",
              },
            ].map((a, i) => (
              <div className="col-6 col-md-3 text-center" key={i}>
                <div className="p-3 border rounded-3 bg-light h-100">
                  <span className="fs-3 mb-1 d-block">{a.icon}</span>
                  <strong className="small text-dark">{a.label}</strong>
                  <br />
        ...
        <h4 className="fw-bold mt-5 mb-4">
          🕒 Chi tiết Bảng giá dịch vụ (Rates Table)
        </h4>
        <div className="row g-4 mb-4">
          {(() => {
            const hourlySections = vehicleTypes
              .map((vt) => {
                const vtPolicies = activeHourlyPolicies.filter(
                  (p) =>
                    String(
                      p.vehicleType?.vehicleTypeId ||
                        p.vehicleTypeId ||
                        p.vehicleType?.id,
                    ) === String(vt.vehicleTypeId),
                );
                if (vtPolicies.length === 0) return null;

                const isMotor =
                  vt.typeName?.toLowerCase().includes("máy") ||
                  vt.typeName?.toLowerCase().includes("moto") ||
                  vt.typeName?.toLowerCase().includes("đạp");
                const displayTypeName = getVietnameseVehicleTypeName(
                  vt.typeName,
                );
                ...
                return {
                  ...
                  desc: isMotor
                    ? "Áp dụng cho mọi loại xe máy và xe đạp điện."
                    : "Áp dụng cho xe ô tô tại mọi điểm đỗ chính.",
                  rows: vtPolicies.map((p) => {
                    const baseMin = p.baseDurationMinutes || 0;
                    const baseH =
                      baseMin >= 60
                        ? `${Math.round(baseMin / 60)} giờ`
                        : `${baseMin} phút`;
                    return [
                      p.policyName,
                ...
                : [
                    {
                      ...
                      desc: "Áp dụng cho xe từ 4 - 7 chỗ tại mọi điểm đỗ chính.",
                      rows: [
                        [
                          "Vé lượt ô tô tiêu chuẩn",
                          `${basePrice.toLocaleString("vi-VN")} VNĐ / 2 giờ`,
                          `Mỗi giờ tiếp theo: ${Math.floor(basePrice * 0.5).toLocaleString("vi-VN")} VNĐ`,
                        ],
                        [
                          "Gửi qua đêm (sau 0h)",
                          `${Math.floor(basePrice * 5).toLocaleString("vi-VN")} VNĐ`,
                          "Giá cố định",
                        ],
                      ],
                      canBook: true,
                    },
                    {
                      type: "Xe máy",
                      icon: "🛵",
                      badge: "bg-secondary text-secondary border-secondary",
                      desc: "Áp dụng cho mọi loại xe máy và xe đạp điện.",
                      rows: [
                        ["Sáng (06h - 18h)", "5.000 VNĐ", "Giá cố định"],
                        ["Tối (18h - 06h)", "8.000 VNĐ", "Giá cố định"],
                        ["Cả ngày (24h)", "12.000 VNĐ", "Giá cố định"],
                      ],
                      canBook: false,
                    },
                  ];
              ...
              <div className="col-md-6" key={i}>
                <div className="card border-0 shadow-sm p-4 rounded-4 bg-white h-100 d-flex flex-column">
                  <div className="d-flex align-items-center gap-2 mb-3">
                    <span className="fs-4">{r.icon}</span>
                    <h5 className="fw-bold m-0">Dành cho {r.type}</h5>
                    <span
                      className={`badge bg-opacity-10 border border-opacity-25 ms-auto ${r.badge}`}
                    >
                      {r.type}
                    </span>
                  </div>
                  ...
                  <div className="d-flex flex-column gap-3 mb-4 flex-grow-1">
                    {r.rows.map((row, idx) => (
                      <div
                        className={`d-flex justify-content-between pb-2 ${idx < r.rows.length - 1 ? "border-bottom" : ""}`}
                        key={idx}
                      >
                        <div>
                  ...
                  <button
                    disabled={!r.canBook}
                    onClick={() => r.canBook && handleBookNow("Standard")}
                    className={`vin-btn vin-btn--full rounded-3 py-2.5 ${r.canBook ? "vin-btn--primary" : "vin-btn--secondary"}`}
                    style={{ cursor: r.canBook ? "pointer" : "not-allowed" }}
                  >
                    {r.canBook
                      ? "Đặt chỗ ngay"
        ...
        <h5 className="fw-bold mt-5 mb-3">
          Gói Đăng Ký Dài Hạn (Subscriptions)
        </h5>
        <p className="text-muted small mb-4">
          Tiết kiệm hơn với các lựa chọn đăng ký theo tháng dành riêng cho
          phương tiện của bạn.
        </p>
            ...
            const displayList =
              activePackages.length > 0
                ...
                : [
                    {
                      policyId: 1,
                      title: "Gói Tháng Tiết Kiệm",
                      name: "Economic Monthly",
                      type: "Economic",
                      price: prices.ecoCar,
                      mP: prices.ecoMotor,
                      desc: "Tiết kiệm chi phí gửi xe định kỳ hàng tháng.",
                      perks: [
                        "Đỗ xe không giới hạn lượt ra vào",
                        "Áp dụng cho mọi vị trí đỗ phổ thông",
                        "Thanh toán tự động qua App",
                    ...
                    {
                      policyId: 3,
                      title: "Gói VIP Cư Dân",
                      name: "VIP Monthly",
                      type: "VIP",
                      price: prices.vipCar,
                      mP: prices.vipMotor,
                      desc: "Đặc quyền đỗ xe cao cấp và dịch vụ chăm sóc trọn gói.",
                      perks: [
                        "Vị trí đỗ ưu tiên gần thang máy",
                        "Hỗ trợ rửa xe 2 lần/tháng",
                        "Miễn phí sạc EV (áp dụng cho 50kWh đầu)",
            ...
            return displayList.map((s, idx) => {
              ...
              return (
                <div className="col-md-6" key={s.policyId || idx}>
                  <div
                    className="card p-4 rounded-4 bg-white h-100 d-flex flex-column position-relative"
                    style={{
                      border: isVip
                        ? "2px solid #164e63"
                        : "1px solid var(--vin-border)",
                  ...
                  >
                    {isVip && (
                      <div
                        className="position-absolute px-3 py-1 bg-primary text-white fw-bold text-uppercase"
                        style={{
                          top: 0,
                          right: 24,
                          fontSize: "0.68rem",
                      ...
                      {s.mP ? (
                        <>
                          <div className="d-flex justify-content-between mb-2">
                            <span className="text-muted small">🛵 Xe máy:</span>
                            <strong
                              className="fs-5"
                              style={{
                                color: "#164e63",
                          ...
                          <div className="d-flex justify-content-between">
                            <span className="text-muted small">🚗 Ô tô:</span>
                            <strong
                              className="fs-5"
                              style={{
                                color: "#164e63",
                    ...
                    <button
                      onClick={() => {
                        ...
                        navigate("/user-dashboard", {
                          state: { tab: "vehicles", autoSubscribePackage: s },
                        });
                      }}
        ...
        <h4 className="fw-bold mt-5 mb-3">
          Thông tin cần lưu ý (Good to Know)
        </h4>
        <div className="row g-3">
          {[
            {
              ...
              desc: "Hủy đặt chỗ miễn phí trước thời gian nhận xe tối thiểu 1 tiếng. Số tiền sẽ được hoàn trả lại cho quý khách trong vòng 24 giờ.",
            },
            {
              icon: "📲",
              title: "Check-in Instructions",
              desc: "Đơn giản chỉ cần lái xe đến cổng rào chắn. Camera nhận diện AI sẽ quét biển số xe đã đăng ký và tự động mở barie. Không cần in vé giấy.",
            },
            {
              icon: "ℹ️",
              title: "Important Notes",
              desc: "Chiều cao xe tối đa giới hạn là 2.1m. Nghiêm cấm các loại phương tiện chở khí gas hóa lỏng LPG đi vào hầm. Hệ thống an ninh tuần tra liên tục bảo đảm an toàn.",
            },
          ].map((g, i) => (
            <div className="col-md-4" key={i}>
              <div className="card border-0 shadow-sm p-4 rounded-4 bg-white h-100">
                <span className="fs-3 mb-2">{g.icon}</span>
                <h6 className="fw-bold">{g.title}</h6>
                <p className="text-muted small mb-0">{g.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        {activeSubPlan && (
          ...
          >
            <div
              ...
              style={{
                maxWidth: subModalStep === 2 ? 1000 : 750,
                maxHeight: "90vh",
              }}
            >
                ...
                <>
                  <div className="mb-4">
                    ...
                    <p className="text-muted small">
                      Hoàn tất các thông tin bên dưới để kích hoạt gói dịch vụ
                      của bạn.
                    </p>
                  </div>

                  <div className="border rounded-3 p-4 mb-4 bg-white shadow-sm">
                    <span className="badge bg-info bg-opacity-10 text-info fw-bold mb-2 px-3 py-1 text-uppercase">
                      PLAN SELECTED
                    </span>
                    <div className="d-flex justify-content-between align-items-center">
                      ...
                      <div className="text-end">
                        <h4 className="fw-bold m-0">
                          {activePlanPrice.toLocaleString("vi-VN")} VNĐ
                        </h4>
                    ...
                    <div className="row g-3">
                      <div className="col-md-6">
                        ...
                        <select
                          ...
                          defaultValue={currentLot.name}
                        >
                          <option>{currentLot.name}</option>
                        </select>
                      </div>
                      <div className="col-md-6">
                        <label className="form-label text-muted small fw-bold">
                        ...
                        compatibleVehicles.map((v) => (
                          <div className="col-md-6" key={v.vehicleId || v.id}>
                            <div
                              ...
                              onClick={() =>
                                setSelectedSubVehicleId(v.vehicleId || v.id)
                              }
                            >
                              <div>
                                <strong
                      ...
                      {compatibleVehicles.length === 0 && (
                        <div className="col-12">
                          <div className="alert alert-warning small mb-0">
                            Bạn chưa có phương tiện phù hợp với gói này. Vui
                            lòng thêm xe mới.
                          </div>
                        </div>
                      )}
                      ...
                      <div className="col-md-6">
                        ...
                        >
                          ...
                          <input
                            type="radio"
                            className="form-check-input mt-0"
                            checked={selectedSubVehicleId === "new"}
                            readOnly
                            style={{ cursor: "pointer" }}
                          />
                        </div>
                  ...
                  <div className="mt-4 text-end">
                    <button
                      className="btn text-white px-5 py-2.5 rounded-3 fw-bold"
                ...
                <div className="row g-5 pb-5 position-relative">
                  <div className="col-lg-7">
                    ...
                    <div className="d-flex flex-column gap-3">
                      {[
                        {
                          id: "vnpay",
                          img: "https://vincheck.vn/wp-content/uploads/2021/05/logo-vnpay.png",
                          t: "VNPAY QR",
                          d: "Quét mã QR từ ứng dụng",
                        },
                        {
                          id: "card",
                          txt: "💳",
                          t: "Thẻ Quốc tế",
                          d: "Visa, Mastercard, JCB",
                        },
                        {
                          id: "cash",
                          txt: "💵",
                          t: "Tiền mặt tại quầy",
                          d: "Thanh toán khi đến nhận chỗ",
                        },
                      ].map((pm) => (
                        <label
                          key={pm.id}
                          className={`border rounded-3 p-3 d-flex justify-content-between align-items-center cursor-pointer shadow-sm`}
                          style={{
                            borderColor:
                              paymentMethod === pm.id ? "#0ea5e9" : "#e2e8f0",
                            borderWidth:
                              paymentMethod === pm.id ? "2px" : "1px",
                          }}
                          onClick={() => setPaymentMethod(pm.id)}
                        >
                          <div className="d-flex align-items-center gap-3">
                            <div
                              className="bg-white rounded d-flex align-items-center justify-content-center border p-1"
                            ...
                            <div>
                              <strong className="d-block m-0">{pm.t}</strong>
                              <small className="text-muted">{pm.d}</small>
                            </div>
                          </div>
                          <input
                            type="radio"
                          ...
                          <div
                            ...
                            style={{
                              width: 22,
                              height: 22,
                              border: `2px solid ${paymentMethod === pm.id ? "#0ea5e9" : "#cbd5e1"}`,
                            }}
                          >
                            {paymentMethod === pm.id && (
                              <div className="rounded-circle w-100 h-100 bg-info"></div>
                            )}
                          </div>
                        </label>
                      ))}
                  ...
                  <div className="col-lg-5">
                    <div className="border rounded-3 p-4 bg-white shadow-sm mt-3">
                      <h6 className="fw-bold mb-4 pb-3 border-bottom d-flex align-items-center gap-2">
                        🧾 CHI TIẾT THANH TOÁN
                      </h6>
                      <div className="d-flex justify-content-between mb-4">
                        <span className="text-muted">Tạm tính</span>
                        <strong>
                          {activePlanPrice.toLocaleString("vi-VN")}đ
                        </strong>
                      </div>
                      <div className="d-flex justify-content-between mb-4 border-bottom pb-4">
                        <span className="text-muted">VAT (10%)</span>
                        <strong>
                          {Math.floor(activePlanPrice * 0.1).toLocaleString(
                            "vi-VN",
                          )}
                          đ
                        </strong>
                      ...
                      <div className="d-flex justify-content-between mt-3">
                        ...
                        <strong className="fs-4" style={{ color: "#164e63" }}>
                          {Math.floor(activePlanPrice * 1.1).toLocaleString(
                            "vi-VN",
                          )}
                          đ
                        </strong>
                  ...
                  <div
                    className="position-absolute bottom-0 start-0 w-100 bg-white border-top py-3 px-4 d-flex justify-content-between align-items-center"
                    style={{
                ...
                <div className="row justify-content-center g-4 py-2">
                  <div className="col-md-6 col-lg-5">
                    <div className="border rounded-4 bg-white shadow-sm h-100 text-center d-flex flex-column p-4 align-items-center justify-content-center">
                      ...
                      <p className="text-muted small">
                        Hệ thống đã ghi nhận đăng ký gói tháng cho xe{" "}
                        <strong>{submittedLicensePlate}</strong>.
                      </p>
                    </div>
                  </div>
                  <div className="col-md-6 col-lg-5 d-flex flex-column gap-3">
                    <div className="border rounded-3 p-4 bg-white shadow-sm">
                      <h6 className="text-muted fw-bold mb-3 small">
                        THÔNG TIN THANH TOÁN
                      </h6>
                      <div className="d-flex align-items-start gap-3 mb-2">
                        <MapPin size={18} className="text-muted" />
                        <div>
                          <small className="text-muted d-block">Bãi đỗ</small>
                    ...
                    {paymentMethod === "vnpay" || paymentMethod === "card" ? (
                      paymentUrl ? (
                        <div className="text-center">
                          <p className="small text-muted mb-3">
                            Vui lòng nhấn nút bên dưới để chuyển hướng đến cổng
                            thanh toán VNPay để hoàn tất.
                          </p>
                          <button
                            type="button"
                            onClick={() => {
                          ...
                          >
                            <img
                              src="https://vincheck.vn/wp-content/uploads/2021/05/logo-vnpay.png"
                              alt="VNPay"
                              style={{
                                height: 20,
                                width: "auto",
                        ...
                        >
                          <div className="small">
                            <strong>
                              ⚠️ Không thể kết nối cổng thanh toán online
                            </strong>
                            <br />
                            Yêu cầu đã được lưu. Vui lòng đến quầy quản lý của
                            chi nhánh để thanh toán và nhận thẻ.
                          </div>
                        </div>
                      ) : (
                      ...
                      >
                        <div className="small">
                          <strong>ℹ️ Đăng ký trả sau / trả trực tiếp</strong>
                          <br />
                          Vui lòng đến quầy quản trị bãi xe để nộp tiền mặt và
                          kích hoạt thẻ tháng. Hotline hỗ trợ: 1900 8868.
                        </div>
                      </div>
                    )}
                    ...
                    <button
                      onClick={handleCloseSubModal}
                      className="btn btn-outline-secondary w-100 fw-bold py-2 rounded-3 mt-2"
                    >
                      Đóng cửa sổ
                    </button>
                  </div>
